...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
    ...
    // Queue Listing
    private readonly SHIPMENT_QUEUE = 'shipment.queue';
    ...
    private async setupInitialCreation() {
        ...
        await this.setupExchangeQueueAndBind(this.SHIPMENT_QUEUE, this.USER_EXCHANGE, '', ExchangeTypeEnum.FANOUT);
        await this.setupExchangeQueueAndBind(this.SHIPMENT_QUEUE, this.SALE_EXCHANGE, '', ExchangeTypeEnum.FANOUT);
        await this.setupExchangeQueueAndBind(this.SHIPMENT_QUEUE, this.BILLING_EXCHANGE, '', ExchangeTypeEnum.FANOUT);

        await this.setupRetryQueue(this.SHIPMENT_QUEUE);
    }
                ...
                async (msg) => {
                    ...
                    try {
                        ...
                        if (requeueTry + 1 < maxRequeues) {
                            ...
                            const retryQueue = `${this.SHIPMENT_QUEUE}.retry`;
