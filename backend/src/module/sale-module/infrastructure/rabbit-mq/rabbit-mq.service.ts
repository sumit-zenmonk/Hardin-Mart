...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
    ...
    // Queue Listing
    private readonly SALE_QUEUE = 'sale.queue';
    ...
    private async setupInitialCreation() {
        ...
        await this.setupExchangeQueueAndBind(this.SALE_QUEUE, this.USER_EXCHANGE, '', ExchangeTypeEnum.FANOUT);
        await this.setupExchangeQueueAndBind(this.SALE_QUEUE, this.BILLING_EXCHANGE, '', ExchangeTypeEnum.FANOUT);
        await this.setupExchangeQueueAndBind(this.SALE_QUEUE, this.SHIPPING_EXCHANGE, '', ExchangeTypeEnum.FANOUT);
        await this.setupExchangeQueueAndBind(this.SALE_QUEUE, this.BILLING_EXCHANGE, '', ExchangeTypeEnum.FANOUT);

        await this.setupRetryQueue(this.SALE_QUEUE);
    }
                ...
                async (msg) => {
                    ...
                    try {
                        ...
                        if (requeueTry + 1 < maxRequeues) {
                            ...
                            const retryQueue = `${this.SALE_QUEUE}.retry`;
